// WahlSoBlue.cpp : Defines the entry point for the application.
//
...
namespace WahlBlues {
	...
	int discoverDevices(BTDevice* &devices) {
		...
		flags |= LUP_FLUSHCACHE | LUP_RETURN_NAME | LUP_RETURN_ADDR | LUP_RETURN_COMMENT;
        ...
        while (!done) {
            ...
            if (NO_ERROR == result) {
                ...
                BTDevice device = {address, port, name, descr};
